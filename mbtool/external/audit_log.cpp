...
static int open_log(FILE **file, const char *path, int flags, const char *fmode, mode_t mode)
{
    ...
    *file = fopen(path, fmode);
    if(!*file) {
        rc = -errno;
        LOGE("Could not open audit log file %s : %s", path, strerror(errno));
        return rc;
    }
    ...
    if (rc != 0) {
        rc = -errno;
        LOGE("Could not setvbuf the log file");
        goto err;
    }
    ...
    if (rc < 0) {
        rc = -errno;
        LOGE("Could not fchmod the log file");
        goto err;
    }
    ...
    if (rc < 0) {
        rc = -errno;
        LOGE("Could not fcntl the log file");
        goto err;
    }
...
audit_log *audit_log_open(const char *logfile)
{
    int rc;
    audit_log *l = NULL;

    l = (audit_log *) calloc(sizeof(struct audit_log), 1);
    if (!l) {
        LOGE("Out of memory while allocating audit log");
        return NULL;
    }
    ...
    if (!l->logfile) {
        LOGE("Out of memory while duplicating logfile string");
        goto err;
    }
...
int audit_log_write(audit_log *l, const char *fmt, ...)
{
    ...
    if(rc < 0) {
        LOGE("Error writing to log file");
        clearerr(l->file);
        rc = -EINVAL;
        goto out;
    }
...
int audit_log_put_kmsg(audit_log *l)
{
    ...
    if (len < 0) {
        rc = -errno;
        LOGE("Could not read kernel log length: %s", strerror(errno));
        return rc;
    }
    ...
    if (!buf) {
        LOGE("Out of memory wile allocating kmsg buffer");
        return -ENOMEM;
    }
    ...
    if (rc < 0) {
        rc = -errno;
        LOGE("Could not read kernel log data: %s", strerror(errno));
        goto err;
    }
        ...
        if (audit) {
            ...
            *audit++ = '\0';
            ...
            /* It contined the AUDIT_KEWORD but was not formatted as expected, just dump it */
                ...
                LOGW("Improperly formatted kernel audit message, dumping as is");
